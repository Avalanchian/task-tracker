...
func (cli *CLI) Act() error {
	...
	case "list":
		...
		cli.OutMsg = List(cli.Flags, cli.Entries)
	case "delete":
	...
	case "mark":
		...
		cli.Entries, cli.OutMsg, err = Mark(markSet.Args(), cli.Flags, cli.Entries)
		if err != nil {
...
func (cli *CLI) createSubCommands() {
	...
	cli.Actions["list"].BoolVar(&cli.Flags.inProgress, "progress", false, "in progress")
	cli.Actions["list"].BoolVar(&cli.Flags.toDo, "todo", false, "todo")
	cli.Actions["list"].BoolVar(&cli.Flags.done, "done", false, "done")

	cli.Actions["mark"].BoolVar(&cli.Flags.inProgress, "progress", false, "in progress")
	cli.Actions["mark"].BoolVar(&cli.Flags.toDo, "todo", false, "todo")
	cli.Actions["mark"].BoolVar(&cli.Flags.done, "done", false, "done")
}
